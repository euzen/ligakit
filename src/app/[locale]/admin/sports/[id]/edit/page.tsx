...
import { EditSportForm } from "./form";

interface PageProps {
  params: Promise<{ locale: string; id: string }>;
}

export default async function SportEditPage({ params }: PageProps) {
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <main className="max-w-2xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-8 space-y-6">
        <Breadcrumbs items={[
          ...
          { label: cs ? "Upravit" : "Edit" },
        ]} />
        <EditSportForm
          locale={locale}
          sportId={id}
          initialName={sport.name}
          initialDescription={sport.description ?? ""}
          initialIcon={sport.icon ?? ""}
