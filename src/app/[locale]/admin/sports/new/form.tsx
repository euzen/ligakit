...
export function NewSportForm({ locale }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setError(data.error || (cs ? "Chyba při vytváření." : "Error creating sport."));
        return;
      }
      const sport = await res.json();
      router.push(`/${locale}/admin/sports/${sport.id}`);
      router.refresh();
    } catch {
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex items-center gap-3">
          <div className="p-2.5 rounded-xl bg-blue-100">
            <Dumbbell className="size-5 text-blue-700" />
          </div>
          <div>
            <h1 className="text-xl font-extrabold text-slate-900">
              {cs ? "Nový sport" : "New Sport"}
            </h1>
            <p className="text-sm text-slate-500">
              {cs ? "Přidejte nový sport do systému" : "Add a new sport to the system"}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        <div className="bg-blue-700 px-6 py-5">
          ...
          <p className="text-blue-200 text-sm mt-0.5">
            {cs ? "Vyplňte údaje o novém sportu" : "Fill in the details for the new sport"}
          </p>
        </div>
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          ...
          <div>
            <label className={labelCls}>
              {cs ? "Název sportu" : "Sport name"} *
            </label>
            <input
              type="text"
              placeholder={cs ? "Fotbal, Basketbal, Tenis..." : "Football, Basketball, Tennis..."}
              value={form.name}
          ...
          <div>
            ...
            <textarea
              placeholder={cs ? "Krátký popis sportu..." : "Short description of the sport..."}
          ...
          <div className="flex gap-3 pt-2">
            ...
            >
              ...
              {cs ? "Vytvořit sport" : "Create sport"}
            </button>
            <a
              href={`/${locale}/admin/sports`}
